...
export default function AutomationsPage() {
  ...
  async function toggleActive(a: Automation, next: boolean) {
    ...
    if (!res.ok) {
      ...
      toast.error(body?.error ?? t("updateFailed"))
      return
    }
  ...
  async function duplicate(a: Automation) {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t("duplicateFailed"))
      return
    }
  ...
  async function confirmDelete() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t("deleteFailed"))
      return
    }
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            {TEMPLATE_ORDER.map((slug) => {
              const tpl = automationTemplates[slug]
      ...
      <Dialog open={!!pendingDelete} onOpenChange={(v) => !v && setPendingDelete(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t("deleteDialogDescriptionPrefix")}{" "}
              <span className="text-foreground">{pendingDelete?.name}</span>{" "}
              {t("deleteDialogDescriptionSuffix")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {automation.is_active && (
              <span className="relative flex h-2 w-2" aria-label={tBuilder("active")}>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>{t("lastRun", { relative: formatRelative(automation.last_executed_at, tRelative) })}</span>
          </div>
        </button>
        ...
        <div className="flex items-center gap-3">
          <Switch
            ...
            aria-label={automation.is_active ? t("deactivateAria") : t("activateAria")}
          />
