 ...
 * `inactivity` automations' configured threshold and dispatches the
 * trigger. Meant to be hit on the same schedule as the other
 * automation crons (external pinger) — same shared-secret auth as
 * /api/automations/cron and /api/nps/cron.
 *
 * The scan cutoff per account is the MINIMUM `hours` among that
 * account's active `inactivity` automations, so a single pass covers
 * every one of them; the engine's triggerMatches() then filters each
 * automation against its own configured hours (and dedupes repeat
 * firings for the same quiet period) once runAutomationsForTrigger
 * evaluates it — see src/lib/automations/engine.ts.
 */
