...
export function WhatsAppConfig() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (data.registered === false && data.registration_error) {
        ...
        toast.success(t('credentialsSavedSkipped'), { duration: 10000 });
        setPin('');
      } else {
        toast.success(
  ...
  async function handleVerifyRegistration() {
    ...
    try {
      ...
      if (data.live) {
        toast.success(t('numberFullyWired'));
      } else {
        toast.error(t('numberNotFullyRegistered'), { duration: 8000 });
      }
      if (accountId) await fetchConfig(accountId);
    } catch (err) {
  ...
  return (
    <section className="animate-in fade-in-50 duration-200">
      ...
      <div className="space-y-6">
        ...
        {showResetBanner && (
          <Alert className="bg-gold-soft border-gold/40">
            <div className="flex items-start gap-3">
              <AlertTriangle className="size-5 text-gold mt-0.5 shrink-0" />
              <div className="flex-1">
                <AlertTitle className="text-gold mb-1">
                  {t('tokenCannotDecryptTitle')}
                </AlertTitle>
        ...
        <Alert className="bg-card border-border">
          ...
          <AlertDescription className="text-muted-foreground">
            {connectionStatus === 'connected'
              ? t('credentialsValidHint')
              : statusMessage || t('connectHint')}
          </AlertDescription>
        </Alert>
          ...
          >
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <AlertTitle
                  className={
                    'mb-0 ' + (isRegistered ? 'text-primary' : 'text-gold')
                  }
                >
                  {isRegistered
                    ? t('registeredTitle')
            ...
            <AlertDescription className="text-muted-foreground mt-2 text-xs leading-relaxed">
              {isRegistered ? (
                <>
                  {t('subscribedSincePrefix')}{' '}
                  {config.registered_at
                    ? new Date(config.registered_at).toLocaleString()
                    : t('unknownDate')}
                  . {t('subscribedSinceMiddle')} <strong>{t('verifyWithMeta')}</strong>{' '}
                  {t('subscribedSinceSuffix')}
                </>
              ) : lastRegistrationError ? (
                <>
                  {t('lastAttemptFailedPrefix')}{' '}
                  <span className="text-destructive">
                    &quot;{lastRegistrationError}&quot;
                  </span>
                  {t('lastAttemptFailedSuffix')}
                </>
              ) : (
                t('neverRegisteredHint')
              )}
            </AlertDescription>
            ...
            {registrationProbe && (
              <div className="mt-3 rounded border border-border bg-card/60 px-3 py-2 space-y-1.5 text-[11px]">
                <p className="font-medium text-foreground">
                  {t('diagnosticLastRun')} {' '}
                  <span className={registrationProbe.live ? 'text-primary' : 'text-gold'}>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                {t('pinHintPart1')} <strong className="text-muted-foreground">{t('pinHintInbound')}</strong>{' '}
                {t('pinHintPart2')} <strong className="text-muted-foreground">{t('pinHintProduction')}</strong>{' '}
                {t('pinHintPart3')}{' '}
                <strong className="text-muted-foreground">
                  {t('pinHintPath')}
                </strong>
                {t('pinHintPart4')}{' '}
                <strong className="text-muted-foreground">{t('pinHintTestNumbers')}</strong>{' '}
                {t('pinHintPart5')}
              </p>
            </div>
          </CardContent>
      ...
      <div>
        <Card>
          <CardHeader>
            <CardTitle className="text-foreground text-base">{t('setupInstructionsTitle')}</CardTitle>
            <CardDescription className="text-muted-foreground">
            ...
            <Accordion>
              <AccordionItem className="border-border">
                ...
                <AccordionContent className="text-muted-foreground">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    <li>{t('step1Item1Prefix')} <span className="text-primary">developers.facebook.com</span></li>
                    <li>{t('step1Item2')}</li>
                    <li>{t('step1Item3')}</li>
                    <li>{t('step1Item4')}</li>
                  </ol>
              ...
              <AccordionItem className="border-border">
                ...
                <AccordionContent className="text-muted-foreground">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    <li>{t('step3Item1')}</li>
                    <li>{t('step3Item2Prefix')} <strong className="text-foreground">{t('phoneNumberIdLabel')}</strong></li>
                    <li>{t('step3Item3Prefix')} <strong className="text-foreground">{t('wabaIdLabel')}</strong></li>
                    <li>{t('step3Item4Prefix')} <strong className="text-foreground">{t('accessTokenLabel')}</strong> {t('step3Item4Suffix')}</li>
                  </ol>
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem className="border-border">
                ...
                <AccordionContent className="text-muted-foreground">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    <li>{t('step4Item1')}</li>
                    <li>{t('step4Item2')}</li>
                    <li>{t('step4Item3Prefix')} <strong className="text-foreground">{t('webhookCallbackUrlLabel')}</strong> {t('step4Item3Suffix')}</li>
                    <li>{t('step4Item4Prefix')} <strong className="text-foreground">{t('verifyTokenLabel')}</strong> {t('step4Item4Suffix')}</li>
                    <li>{t('step4Item5')}</li>
                  </ol>
                </AccordionContent>
              </AccordionItem>
