...
export function ProfileForm() {
  ...
  const onPickFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_MIME.has(file.type)) {
      toast.error(t('unsupportedImageType'), {
        description: t('unsupportedImageHint'),
      });
      return;
    }
    if (file.size > MAX_AVATAR_BYTES) {
      toast.error(t('imageTooLarge'), {
        description: t('imageTooLargeHint'),
      });
      return;
    }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(emailSent ? t('profileSavedCheckEmail') : t('profileSaved'));
    } catch (err) {
            ...
            {emailChangePending && (
              <p className="flex items-start gap-2 rounded-md border border-gold/30 bg-gold-soft px-3 py-2 text-xs text-gold">
                ...
                <span>
                  ...
                  {t('emailChangePendingPart2')} <strong>{email}</strong>{' '}
                  {t('emailChangePendingPart3')}
                </span>
              </p>
            )}
          </div>
          ...
          <div className="rounded-lg border border-border bg-muted p-4">
            ...
            <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <dt className="text-muted-foreground">{t('userIdLabel')}</dt>
